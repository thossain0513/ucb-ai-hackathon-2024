...
class Connection:
    ...
    async def connect(
        ...
        while cls.is_running:#not stop_event.is_set():
        #while is_running:
            try:
        ...
        try:
            async for message in socket:
                try:
                    ...
                    if json_message.get("type") == "assistant_message": 
                        cls.transcript.append(json_message)
                        #print("Assistant message:", json_message["message"]["content"])
                    elif json_message.get("type") == "user_message": 
                        cls.transcript.append(json_message)
                        #print("User message:", json_message["message"]["content"])
                    
                    # Check if the message type is 'audio_output'
                    elif json_message.get("type") == "audio_output":
